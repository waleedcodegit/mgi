...
class List extends Component {
    ...
    componentDidMount(){
        Axios.get('/api/get_today_tournaments').then(res=>{
            ...
            this.setState({
                is_display: true,
                todayTournaments:res.data.todayTournaments
            })
        })
    }
    Deletetournament(id){
            ...
            <div className="container">
                  {
                    ...
                    <h2 className="section_title">Today Tournaments List</h2>
                </div>
                ...
                <table className="table table-hover table-bordered table-striped mt-4">
                    ...
                    <tbody>
                        {
                            this.state.todayTournaments.map((data,index)=>{
                                return(
                                    <tr>
                                        <td>{index+1}</td>
                                        <td>{data.title}</td>
                        ...
                        {
                                    this.state.todayTournaments.length == 0 ? 
