...
class List extends Component {
    ...
    componentDidMount(){
        Axios.get('/api/get_all_videos').then(res=>{
            console.log(res);
            this.setState({
                is_display:true,
                            ...
                            this.state.videos.map((data,index)=>{
                                return(
                                    <tr>
                                        ...
                                        <td width="15%">
                                            <Link to={`/adminpanel/edit-video/${data.id}`}><button className="btn btn-success"> <i style={{color:'white'}} className="far fa-edit "> </i></button></Link>
                                            <button className="btn btn-light" onClick={this.DeleteVideo.bind(this,data.id,index)}> <i  style={{color:'red'}}  className="fas fa-trash-alt"></i></button>
                                        </td>
                                    </tr>
                                )
                            })
